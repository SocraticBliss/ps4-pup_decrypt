                            ...
                            uint16_t table_index, pup_segment* table_segment)
{
    ...
    result = encsrv_decrypt_segment(state->device_fd, table_index, table_buffer, table_length);
    if (result != 0) {
        int errcode = errno;
    ...
    for (int i = 0; i < block_count; i++) {
        ...
        result = encsrv_decrypt_segment_block(state->device_fd, index, i, block_buffer, read_size, table_buffer, table_length);
        if (result < 0) {
            int errcode = errno;
...
int decrypt_pup_data(const decrypt_state * state)
{
    ...
    if ((file_header.flags & 1) == 0) {
        ...
        result = encsrv_decrypt_header(state->device_fd, header_data, header_size, state->pup_type);
        if (result != 0) {
            int errcode = errno;
